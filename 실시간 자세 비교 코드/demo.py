...
parser.add_argument('--target_video', type=str, default='/home/piai/CV-pose-detection/media/ex1.mp4') # 빵느 영상
...
target = cv2.VideoCapture(args.target_video) # target video frame 제어
...
framerate = 0
start = time.time()
time.sleep(5)  # delay to wait for detection
...
while True:
    ...
    if type(target_datum.poseKeypoints) != type(None):
