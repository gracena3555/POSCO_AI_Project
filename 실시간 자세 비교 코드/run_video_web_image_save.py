...
while True:
    ...
    #         # Scale, transform, normalize, reshape, predict
        ...
        if type(frame_datum.poseKeypoints) != type(None):
            ...
            #         similarity_score_list = [] # 다시 리스트 초기numpy
    # elif 75 <= count // 25 <= 83:
    #     ordinal_score = (0, '', 0.0, (0, 0, 0))
...
#######image_save
    if ordinal_score[0] > best_flag:
        best_pose = addh
        best_flag = ordinal_score[0]
        print(best_flag)
        if len(best_list) >= 3:
            best_list.sort(key=lambda x:-x[0])  #내림차순 정렬
            best_list.pop()
        best_list.append((best_flag, best_pose))
        print(best_list)


        
    elif ordinal_score[0] > 0.3 and ordinal_score[0] < worst_flag:
        worst_pose = addh
        worst_flag = ordinal_score[0]
        print(worst_flag)
        if len(worst_list) >= 3:
            worst_list.sort(key=lambda x:x[0]) #오름차순정렬
            worst_list.pop()
        worst_list.append((worst_flag, worst_pose))
        print(worst_list)
...
b_count = 0
w_count = 0
for _, b_pose in best_list:
    cv2.imwrite('./best_pose/best'+str(b_count)+'.jpg', b_pose)
    b_count += 1
for _, w_pose in worst_list:
    w_count += 1
    cv2.imwrite('./worst_pose/worst'+str(w_count)+'.jpg', w_pose)
